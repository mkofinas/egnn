...
class EGNN_vel(nn.Module):
    def __init__(self, in_node_nf=1, in_edge_nf=2, hidden_nf=64, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, recurrent=True, norm_diff=False, tanh=False):
    ...
    def forward(self, h, x, edges, vel, edge_attr):
        ...
        for i in range(0, self.n_layers):
            h, x, v = self._modules["gcl_%d" % i](h, edges, x, vel, edge_attr=edge_attr)
        return x, v
...
class E_GCL_vel(nn.Module):
    ...
    def forward(self, h, edge_index, coord, vel):
        ...
        edge_feat = self.edge_model(h[row], h[col], radial, edge_attr=None)
        ...
        h, agg = self.node_model(h, edge_index, edge_feat, node_attr=None)
