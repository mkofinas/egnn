...
class NBodyDatasetAccel():
    ...
    def load(self):
        ...
        acc = np.load('n_body_system/dataset/acc_' + self.sufix + '.npy')
        edges = np.load('n_body_system/dataset/edges_' + self.sufix + '.npy')
        charges = np.load('n_body_system/dataset/charges_' + self.sufix + '.npy')

        loc, vel, acc, edge_attr, edges, charges = self.preprocess(loc, vel, acc, edges,
                                                              charges)
        return (loc, vel, acc, edge_attr, charges), edges


    def preprocess(self, loc, vel, acc, edges, charges):
        ...
        acc = torch.Tensor(acc).transpose(2, 3)
        n_nodes = loc.size(2)
        loc = loc[0:self.max_samples, :, :, :]  # limit number of samples
        vel = vel[0:self.max_samples, :, :, :]  # speed when starting the trajectory
        acc = acc[0:self.max_samples, :, :, :]  # accel when starting the trajectory
        ...
        return torch.Tensor(loc), torch.Tensor(vel), torch.Tensor(acc), torch.Tensor(edge_attr), edges, torch.Tensor(charges)
    ...
    def __getitem__(self, i):
        loc, vel, acc, edge_attr, charges = self.data
        loc, vel, acc, edge_attr, charges = loc[i], vel[i], acc[i], edge_attr[i], charges[i]
        ...
        return loc[frame_0], vel[frame_0], acc[frame_0], edge_attr, charges, loc[frame_T]
